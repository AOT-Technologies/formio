...
module.exports = (formio) => {
  ...
  const componentPaths = (components) => {
    ...
    util.eachComponent(
      components,
      (component, fullPath, components, parent, compPaths) => {
        const path = process.env.FF_FORBID_DUPLICATED_DATA_PATH
          ? compPaths.dataPath
          : compPaths.fullPath;
        const componentInfo = CoreUtils.componentInfo(component);
        if (!componentInfo.layout && !_.isUndefined(component.key) && !_.isNull(component.key)) {
  ...
  const uniqueValidator = (property) =>
