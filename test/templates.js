    ...
    let checkTemplateFormsAndResources = (project, type, input, done) => {
      ...
      formio.resources.form.model.find({type, deleted: {$eq: null}}).then(forms => {
        ...
        forms.forEach(form => {
          ...
          let tempForm = _.omit(form, ['_id', '__v', 'created', 'deleted', 'modified', 'machineName', 'owner', '_vid', 'revisions', 'submissionRevisions', 'isBundle', 'pdfComponents', 'tenantKey', 'parentFormId']);
